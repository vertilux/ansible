...
*** Playbooks
**** Restart Granite
This playbbok wil use ~win_service~ – Manage and query Windows services to restart a chosen Granite service. For beter UX select an =application (country or distribution center)= from the prompt menu.
...
#+begin_src bash
ansible-playbook restart-server.yml --extra-vars="target=granite_dr" --vault-password-file group_vars/vault_pass.txt -i inventory
#+end_src

*Playbook with a group of hosts:*
#+begin_src bash
ansible-playbook restart-server.yml --extra-vars="target=granite" --vault-password-file group_vars/vault_pass.txt -i inventory
#+end_src
...
+ [ ] Refector ERP playbooks
+ [ ] Separate ERP playbooks from non-ERP
